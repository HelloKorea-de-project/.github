...
## 더미데이터 생성 ##
실제 운영되는 서비스에서는 대량의 데이터가 적재될 것으로 예상되어 더미데이터를 생성했습니다. 

항공권 검색/클릭/구매 로그와 사용자 정보, 세션별 채널 & 생성/접속 시각 정보를 생성했습니다.

디멘션 테이블과 팩트 테이블의 구성을 갖는 스타스키마로 구성하였으며, 팩트 테이블의 경우 최대 4억행의 데이터를 갖도록 생성하였습니다.
...
<br>2. 디멘전 테이블로부터 팩트 테이블 생성시 매번 사용자 정보를 샘플링해서 발생시켜 예상 생성 시간이 몇 주 이상으로 상당히 오래걸리는 문제에 직면하였습니다. 이에 검색/클릭/구매 로그와 같은 팩트 테이블을 랜덤한 값으로 먼저 생성하고 사용자 정보와 같은 디멘전 테이블을 팩트 테이블을 통해 생성하는 것으로 해결하였습니다.

<br>3. faker라는 더미데이터 생성 라이브러리를 사용하고 있었는데, faker 사용시 불균등한 현실 데이터와는 달리 균등한 분포의 데이터를 생성한다는 것을 확인했습니다. 이에 현실 데이터처럼 Data Skew를 주기 위해 불균등한 데이터를 임의로 생성해 추가했습니다
...
<br>2. 사용자id별 통계 쿼리(`GROUP BY`)처럼 자주 조회하는 통계 쿼리를 별도의 테이블로 저장해둠으로써, 같은 쿼리 실행시 성능을 향상시키는 방법입니다
...
## Spark ##
Redshift에서 수행했던 대용량 데이터 쿼리들을 Spark에서 수행해 성능을 비교했습니다. 4가지로 최적화 성능을 비교했습니다
...
- 크게 쿼리 최적화 전 셔플 파티션 최적화와 쿼리 최적화 후 셔플 파티션 최적화 실험을 진행했습니다.
  - Glue에서 기본으로 설정된 36개 파티션의 경우 43초가 소요되었으나, 72개 이상의 파티션의 경우 18.7초까지 연산 시간이 감소하는 것을 확인하였습니다.
 
<br>4. "소금뿌리기" 기법을 통한 data skew 개선
- 조인에 참여하는 속성에 난수와 같은 임의의 속성을 추가하여 조인하는 기법으로 이를 통해 편향된 데이터로 인해 발생하는 memory spill 현상을 해결할 수 있었습니다.
